    ...
    try {
        ...
        connection.query(query, values, (err, result) => {
            if (err) {
                ...
                res.status(500).json({ status: 500, message: "An error occurred while adding data" });
            } else {
...
exports.updateTask = async (req, res) => {
    ...
    try {
        ...
        if (image) {
            query = 'UPDATE tasks SET heading = ?, description = ?, date = ?, time = ?, image = ? WHERE id = ?';
            values = [heading, description, date, time, image, taskId];
        } else {
            query = 'UPDATE tasks SET heading = ?, description = ?, date = ?, time = ? WHERE id = ?';
        ...
        connection.query(query, values, (err, result) => {
            if (err) {
                ...
                res.status(500).json({ status: 500, message: "An error occurred while updating data" });
            } else {
...
exports.getTaskById = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'An error occurred while fetching unique task' });
  }
};
